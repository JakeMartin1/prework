...
let nextTimeItem = groceryList.pop(); 
//removes the string "carrots" from the end of groceryList and stores it in the variable nextTimeItem

getNextTime.push(nextTimeItem); 
//pushes the variable nextTimeItem, which contains the string "carrots" onto the end of 
//the getNextTime array

console.log("Now, your grocery list is", groceryList, "and your get next time list is", getNextTime);

/*You also realized you have eggs at home now, but will definitely need to get more next time. 
Because it will be a top priority for that trip, 
you want to add it to the beginning of the getNextTime array*/
nextTimeItem = groceryList.shift(); 
//removes "eggs" from the beginning of the grocery list, 
//nextTimeItem was declared with let, so we can change it here for our new nextTimeItem

getNextTime.unshift(nextTimeItem); 
//adds nextTimeItem, which contains "eggs", to the front of the getNextTime array
...
//You've been persuaded to add beef wellington to the menu, 
//but you really hope no one orders it so you add it to the end of the menu 
//and hopefully no one will see.
//Conversely, you have some really great prawns that you'd like to feature. 
//Add them to the beginning of the menu to highlight them.
...
//You need to know which index position is the halfway point. 
//Determine that by finding the length of the array and dividing by 2
const halfIndex = (menu.length / 2) //add your code on this line

//Use the array slice() method to assign 
//the first half of the menu to Maria
const mariasDishes = menu.slice(0,5)//add your code here
//Use the array slice() method to assign 
//the second half of the menu to Nico
